...
describe('Hacker Stories', () => {
  ...
  context('Hitting the real API', () => {
    ...
    it('searches via the last searched term', () => {
      ...
      cy.getLocalStorage('search')
        .then(search => {
          expect(search).to.equal(newTerm)
        })
      ...
      cy.getLocalStorage('search')
        .then(search => {
          expect(search).to.equal(initialTerm)
        })
  ...
  context('Mocking the API', () => {
    context('Footer and list of stories', () => {
      ...
      context('List of stories', () => {
        it('shows the right data for all rendered stories', () => {
          ...
          cy.contains('a', stories.hits[0].title)
          ...
          cy.contains('a', stories.hits[1].title)
    ...
    context('Search', () => {
      beforeEach(() => {
        cy.intercept('GET', `**/search?query=${initialTerm}&page=0`, { fixture: 'emptyStories' })
          .as('getEmptyStories')
      ...
      context('Last searches', () => {
        it('shows a max of 5 buttons for the last searched terms', () => {
          ...
          cy.intercept('GET', '**/search**', { fixture: 'emptyStories' }).as('getRandomStories')
...
it('shows a "Loading ..." state before showing the results', () => {
  ...
  cy.intercept('GET', '**/search**', {
    delay: 1000,
    fixture: 'stories'
  }).as('getDelayesStories')

  cy.visit('/')

  cy.assertLoadingIsShownAndHidden()

  cy.wait('@getDelayesStories')
